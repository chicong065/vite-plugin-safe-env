...
describe('blockOn: production (default)', () => {
  afterEach(() => {
    vi.unstubAllEnvs()
  })

  it('aborts the build when NODE_ENV is production', async () => {
  ...
  it('completes the build with a warning when NODE_ENV is development', async () => {
    vi.stubEnv('DATABASE_URL', 'postgres://user:secret@localhost/db')
